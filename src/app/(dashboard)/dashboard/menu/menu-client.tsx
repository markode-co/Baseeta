    ...
    <main className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden">
      <div className="bg-white border-b border-slate-200 px-3 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-2" dir="rtl">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500">{items.length} صنف · {cats.length} فئة</p>
        </div>
              ...
              {filtered.map((item) => (
                <Card key={item.id} className={`overflow-hidden ${!item.isAvailable ? "opacity-60" : ""}`}>
                  ...
                  <CardContent className="p-3">
                    ...
                    <div className="flex gap-1.5 sm:gap-2 pt-2 border-t border-slate-100">
                      <Button variant="outline" size="sm" className="flex-1 px-2" onClick={() => openEditItem(item)}>
                        <Edit className="w-3.5 h-3.5" />
                        <span className="hidden sm:inline">تعديل</span>
                      </Button>
                      <Button variant="destructive" size="sm" className="px-2.5" onClick={() => deleteItem(item.id)}>
